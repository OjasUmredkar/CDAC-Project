...
@Service
@Transactional
public class TicketSlotServiceImp implements TicketSlotService {
	...
	public TicketSlot getById(Long id) {
		...
		return tickRepo.findById(id).orElseThrow(()-> new ResourceNotFoundException("Invalid id"));
	}

	@Override
	public List<TicketSlot> getSlotByMonId(long id) {
		Monument m=monumentService.getMonumentById(id);
		
		return tickRepo.findByMonument(m);
	}
